...
import sys, optparse, serial, struct, time, datetime, re, signal, RPi.GPIO as GPIO
...
YELLOW_LED = 12
GREEN_LED = 13
DISAPPEAR_DELAY_TIME = 200 #Buffer time <ms> before "Tag disappear" RED light
...
"""
    ...
    #Team SuPear LED handling
    global led_status, led_counter, RED_LED, YELLOW_LED, GREEN_LED
    ...
    former_led_status = 0
    led_status = RED_LED #start always with red led
    ...
    while (1):
        ...
        #Team SuPear LED-control
        if led_counter < DISAPPEAR_DELAY_TIME:
            led_counter = led_counter + 1
        else:
            led_status = RED_LED
        
        if led_status != former_led_status:
            GPIO.output(RED_LED,GPIO.LOW)
            GPIO.output(YELLOW_LED,GPIO.LOW)
            GPIO.output(GREEN_LED,GPIO.LOW)
            GPIO.output(led_status,GPIO.HIGH)
            former_led_status = led_status
                        ...
                        if bytes_left == 0:
                            ...
                            if bytes_left == 0:
                                ...
                                if this_field[0] == 0x07: # BGLIB_GAP_AD_TYPE_SERVICES_128BIT_ALL
                                    ...
                                    if this_field[5] == 0: #With proto 0 only first value is needed to check, 0 = Moving, 201 = Still
                                        led_status = GREEN_LED
                                        print "Moving"
                                    elif this_field[5] == 201:
                                        led_status = YELLOW_LED
                                        print "Still"
                                    led_counter = 0 #TAG found so counter zeroed
